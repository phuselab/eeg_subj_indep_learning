...
if __name__ == '__main__':
    ...
    torch.save(dict(data=torch.from_numpy(data), subjects=torch.from_numpy(subjects), tasks=torch.from_numpy(tasks), runs=torch.from_numpy(runs), labels=para_label_names, data_mean=torch.from_numpy(data_mean), data_std=torch.from_numpy(data_std)), './data/processed_data/'+('full' if full else 'simple')+'_erp_data.pt')
