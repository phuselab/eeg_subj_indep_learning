...
class Trainer(object):
    ...
    def train_for_multiclass(self):
        ...
        for epoch in range(self.params.epochs):
            ...
            for x, y in tqdm(self.data_loader['train'], mininterval=10):
                ...
                y = y.cuda()
                pred = self.model(x)
                # print the histogram of pred
                pred_cpu = pred.detach().cpu()
                # print("Range of predictions:", pred_cpu.min().item(), pred_cpu.max().item())
                # print(f'All predictions: {pred_cpu.argmax(dim=1)}')
                # binned , _ = torch.histogram(pred_cpu, bins=4, range=(0, 1.0))
                # print("Histogram of predictions:", binned.numpy())
                # softmaxed = torch.softmax(pred, dim=1)
                # softmaxed_cpu = softmaxed.detach().cpu()
                # binned_softmax, _ = torch.histogram(softmaxed_cpu, bins=4, range=(0, 1.0))
                
                # print("Histogram of softmaxed predictions:", binned_softmax.numpy())
        ...
        with torch.no_grad():
            ...
            wandb.log({
                    ...
                    "Test/kappa": kappa,
                    "Test/f1": f1,
                    "best_val_kappa": kappa_best # Log the metric that triggered model saving
                })
    ...
    def train_for_binaryclass(self):
        ...
        with torch.no_grad():
            ...
            if not os.path.isdir(self.params.model_dir):
                os.makedirs(self.params.model_dir)
            model_path = self.params.model_dir + "/epoch{}_acc_{:.5f}_pr_{:.5f}_roc_{:.5f}_wandb.pth".format(best_f1_epoch, acc, pr_auc, roc_auc)
            torch.save(self.model.state_dict(), model_path)
            print("model save in " + model_path)
