...
# ── Global Environment ───────────────────────────────────────
PROJECT_NAME="openMIIR"
BASE_SAVE_DIR="../../../experiments/openMIIR"
MAIN_SCRIPT="../../../main.py"
DATA_FILE_NAME="openmiir_force64_3.pt"
...
run_group_2() {
    ...
    local batch_size=128
    local cls_type="--classifier-type diva_classifier"

    # run_experiment "2A" "${model}" "${config_dir}/A_task_only.yaml" "${epochs}" "${stage1_epochs}" "${analysis_block}" "${batch_size}" ${cls_type}
    # run_experiment "2B" "${model}" "${config_dir}/B_task_subj.yaml" "${epochs}" "${stage1_epochs}" "${analysis_block}" "${batch_size}" ${cls_type}
    # run_experiment "2C" "${model}" "${config_dir}/C_task_subj_noise_variational.yaml" "${epochs}" "${stage1_epochs}" "${analysis_block}" "${batch_size}" ${cls_type}
    run_experiment "2D" "${model}" "${config_dir}/D_generator_reconstruction.yaml" "${epochs}" "${stage1_epochs}" "${analysis_block}" "${batch_size}" ${cls_type}
    # run_experiment "2E" "${model}" "${config_dir}/E_discriminator_full_DIVA.yaml" "${epochs}" "${stage1_epochs}" "${analysis_block}" "64" ${cls_type}
    # run_experiment "2F" "${model}" "${config_dir}/F_DIVA_no_UNET.yaml" "${epochs}" "${stage1_epochs}" "${analysis_block}" "64" ${cls_type}
